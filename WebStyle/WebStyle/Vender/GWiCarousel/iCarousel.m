...
//    return 1.0 - MIN(factor, fadeRange) / fadeRange * (1.0 - fadeMinAlpha);
    CGFloat defaultAlpha = 1.0 - MIN(factor, fadeRange) / fadeRange * (1.0 - fadeMinAlpha);
    if([self.delegate respondsToSelector:@selector(carousel:alphaForItemWithOffset:withDefault:)])
    {
        return [_delegate carousel:self alphaForItemWithOffset:offset withDefault:defaultAlpha];
    }
...
- (id)viewOrSuperview:(UIView *)view ofClass:(Class)viewClass
{
    if (!view || view == self.contentView)
    {
        return nil;
    }
    else if ([view isKindOfClass:viewClass])
    {
        return view;
    }
    return [self viewOrSuperview:view.superview ofClass:viewClass];
}
